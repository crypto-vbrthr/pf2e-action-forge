...
test("dev.17 release metadata is synchronized", async () => {
  const manifest = await readJson("module.json");
  const pkg = await readJson("package.json");
  assert.equal(manifest.version, "0.1.0-dev.17");
  assert.equal(pkg.version, "0.1.0-dev.17");
  assert.equal(manifest.download, "https://github.com/crypto-vbrthr/pf2e-action-forge/releases/download/v0.1.0-dev.17/pf2e-action-forge.zip");
});
