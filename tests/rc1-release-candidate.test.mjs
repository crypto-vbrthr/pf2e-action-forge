...
test("rc.1 release metadata, runtime fallbacks, and download URL are synchronized", async () => {
  ...
  assert.match(bootstrap, /0\.1\.0-rc\.1/);
  assert.match(app, /0\.1\.0-rc\.1/);
  assert.doesNotMatch(bootstrap, /0\.1\.0-dev\./);
  assert.doesNotMatch(app, /0\.1\.0-dev\./);
});

test("rc.1 catalog remains 65 unique normalized actions with no development-only entries", () => {
...
test("German and English locale key sets remain identical in rc.1", async () => {
