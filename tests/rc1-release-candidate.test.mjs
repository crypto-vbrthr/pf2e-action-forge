...
test("rc.2 release metadata, runtime fallbacks, and download URL are synchronized", async () => {
  ...
  assert.equal(manifest.download, "https://github.com/crypto-vbrthr/pf2e-action-forge/releases/download/v0.1.0-rc.2/pf2e-action-forge.zip");
  assert.match(bootstrap, /0\.1\.0-rc\.2/);
  assert.match(app, /0\.1\.0-rc\.2/);
  assert.doesNotMatch(bootstrap, /0\.1\.0-dev\./);
  assert.doesNotMatch(app, /0\.1\.0-dev\./);
});

test("rc.2 catalog remains 65 unique normalized actions with no development-only entries", () => {
...
test("German and English locale key sets remain identical in rc.2", async () => {
