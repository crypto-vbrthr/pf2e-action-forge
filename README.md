...
**Open an issue here:** https://github.com/crypto-vbrthr/pf2e-action-forge/issues
...
- Release metadata and runtime fallback versions are synchronized to `0.1.0-rc.3.2`.
...
### Roll dialog window stacking

When Action Forge launches a native PF2e check, it temporarily yields its window layer so the PF2e roll configuration dialog remains in front even after repeated rolls. The Forge returns to the foreground after the system dialog closes.
